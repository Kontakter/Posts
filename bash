Последний месяц я программировал на баше и хочу поделиться своими впечатлениями (если вы не знаете, что такое bash дальше можно не читать).

Года два назад мне уже приходилось писать нетривиальный скрипт с использованием bash и это было отвратительно.
В этот раз у меня был достаточно интересный проект, связанный с git-ом, подробностей рассказать не могу.
Но в итоге получился скрипт строк на 100-150 и еще столько же (по объему) тестов к нему.
Общее впечатление таково: на баше тоже можно писать код, в меру эффективный и понятный, но все-таки лучше этого не делать. 

Ниже я хочу обсудить следующие вопросы: почему люди до сих пор используют bash, недостатки этого подхода, и что же делать, если вы все-таки пишите на баше.

Итак, начнем. Меня всегда удивляло две вещи: сколько всякой фигни написано на баше и гордость людей за то, что они умеют производить на свет `баш-скрипты`.
Теперь я удивляюсь этому меньше, потому что начинаю понимать природу этих вещей.
...
Чтобы объяснить подробней смысл сказанного, я расскажу маленькую выдуманную историю, которую тем не менее часто приходится встречать в жизни:
...
При написании скрипта он 90% времени тратит на чтении man-ов и гугление готовых решений.
...
Этот скрипт работает и возможно делает нечто не совсем тривиальное, например,
...
каком-нибудь форуме и бежит хвастаться друзьям.
...
3. Наверняка автор использовал кучу вещей, которые уже стояли у него в системе, или которые он поставил в ходе
   разработки этого скрипта. То есть из коробки ни у кого скрипт работать не будет.
"""
...
4. Используйте поэлементные циклы (!!в таком виде работает только в sh!!):
...
res = `python aux_prog.py < "some text"`

Этот список можно еще долго продолжать, главное всегда помните про нулевой пункт.

Несомненная польза баша состоит лишь в создании вашего bashrc и в быстром и эффективном написании маленьких полезных скриптов, которые упрощают вам жизнь.
Про то, как писать такие скрипты, и что должно лежать в bashrc можно написать еще два длинных поста, поэтому на этом я закончю свой рассказ.

Надеюсь, данный текст было приятно читать, и каждый узнал для себя что-нибудь новое.
